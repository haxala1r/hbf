...
executeOne :: (Char -> IO ()) -> Instr -> Tape -> IO Tape

executeOne _ NextData (Tape ls c (Cons r rs)) =
  return $ Tape (c : ls) r rs

executeOne _ PrevData t = f t
  where
    f (Tape [] c rs) = return $ Tape [] c rs
    f (Tape (l : ls) c rs) = return $ Tape ls l (Cons c rs)
executeOne _ Inc (Tape ls c rs) = return $ Tape ls (c + 1) rs
executeOne _ Dec (Tape ls c rs) = return $ Tape ls (c - 1) rs
...
executeOne f (Loop is) (Tape ls c rs) =
  if c == 0 then
    ...
    do
      t <- executeAll f is (Tape ls c rs)
      executeOne f (Loop is) t

executeAll :: (Char -> IO ()) -> [Instr] -> Tape -> IO Tape
executeAll f is t =
  foldM (\t' i -> executeOne f i t') t is  
